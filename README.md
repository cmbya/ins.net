...
这份配置从公开 GitHub 仓库构建镜像，NAS 首次构建需要连接 GitHub、PyPI 和 Debian 软件源。下载数据保存在 Docker 命名卷 `insnet-data`，重启和更新项目时会保留；**删除项目时不要勾选删除数据卷**。命名卷由 Docker 管理，不会直接显示在飞牛文件管理器的 `/vol2/1000` 中。端口占用时，把 `18080:18080` 左侧改为其他空闲端口，例如 `18081:18080`，浏览器也使用新端口。
