...
- Instagram 授权页管理 Cookie 状态、账号备注、博主作品与已保存帖子的独立保存目录，并可主动检查授权。
- 系统配置可设全局调度开关、新博主默认间隔与上限、已保存帖子调度、默认目录和日志保留天数。
...
6. “同步已保存”可手动运行；在系统配置中设置它的自动同步开关、同步范围、间隔和每轮下载上限。已经归档的帖子会跳过。
7. 在 Instagram 授权页按账号设置 `/archive` 下博主作品与已保存帖子的保存子目录；系统配置里的默认目录供新归档使用。系统日志可查最近任务详情。
